...
Automatically adds YAML frontmatter to Obsidian files based on filename patterns
"""
...
class ObsidianAutoTagger:
    ...
    def extract_file_info(self, filename):
        """Extract information from filename pattern: YYYY-MM-DD_HH-MM-SS_Source_Topic....md"""
        pattern = r'(\d{4}-\d{2}-\d{2})_(\d{2}-\d{2}-\d{2})_(.+?)_(.+)\.md'
        match = re.match(pattern, filename)

        if match:
            date, time, source, topic = match.groups()
            return {
                'date': date,
                'time': time.replace('-', ':'),
...
"""
    ...
    def process_file(self, file_path):
        ...
        try:
            ...
            file_info = self.extract_file_info(file_path.name)
            if not file_info:
                self.skipped_count += 1
            ...
            return f"✅ Tagged: {file_path.name}"
    ...
    def run(self):
        ...
        print("🏷️  Obsidian Auto-Tagger Starting...")
        ...
        if conversations_path.exists():
            print("🔍 Processing AI Conversations...")
        ...
        if self.processed_count > 0:
            print("✨ Auto-tagging complete! Your files now have structured metadata.")
        else:
...
def main():
    ...
    tagger = ObsidianAutoTagger(vault_path)
    tagger.run()
